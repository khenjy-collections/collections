...
<?php $header = $this->db->query("SELECT namaweb FROM tampilan LIMIT 1;"); $hdr = $header->result_array(); ?>
...
<header>
  ...
  <div class="login"><?php if ($this->session->userdata('is_logged_in')) { echo "<a href=" . site_url('user/profil') . ">Profil</a> | <a href=" . site_url('user/logout') . ">Logout</a>"; } else { ?>    <a href="<?php echo site_url('user/daftar'); ?>"><i class="fas fa-sign-in"></i> Daftar</a> | <a href="<?php echo site_url('user/login'); ?>"><i class="fas fa-key"></i> Login</a><?php } ?></div>
</header>
